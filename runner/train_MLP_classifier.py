...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Confidence MLP Classifier")
...
def main():
    ...
    # Initialize wandb
    wandb.init(
        project="confidence_classifier-NMT",
        name = args.run_name,
        config=vars(args)
    )
    ...
    np.savez(f'./output/{args.run_name}_roc_curve.npz', fpr=fpr, tpr=tpr, thresholds=thresholds, roc_auc=roc_auc)
    ...
    plt.tight_layout()
    plt.savefig(f'./output/{args.run_name}_roc_curve.png')
    wandb.log({"roc_curve": wandb.Image(f'./output/{args.run_name}_roc_curve.png')})
    plt.close()
